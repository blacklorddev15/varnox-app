...
const issueToken = (res, userId) => {
  ...
  // sameSite must be "none" for the packaged Android app: its WebView origin is
  // https://localhost, which is cross-site relative to this API, so a "lax" cookie is never
  // sent and login appears to succeed and then immediately fails. SameSite=None requires
  // Secure, which Vercel's HTTPS provides. Override with COOKIE_SAME_SITE=lax if you later
  // serve the web app and API from the same registrable domain and want the tighter setting.
...
const logout = async (req, res) => {
  try {
    // Attributes must match those used when issuing the cookie, or the browser will not
    // clear it. Kept consistent with issueToken().
